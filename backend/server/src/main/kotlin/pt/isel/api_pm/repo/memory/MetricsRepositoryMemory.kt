...
class MetricsRepositoryMemory : MetricsRepository {
    ...
    override suspend fun save(userId: UInt, monitoredEndpointId: UInt, metric: RequestMetric) {
        metrics.getOrPut(userId) { ConcurrentHashMap() }.getOrPut(monitoredEndpointId) { mutableListOf() }.add(metric)
    }

    override suspend fun getByEndpoint(userId: UInt, monitoredEndpointId: UInt): List<RequestMetric> = metrics[userId]?.get(monitoredEndpointId)?.toList() ?: emptyList()
